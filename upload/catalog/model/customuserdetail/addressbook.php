...
class ModelCustomuserdetailAddressbook extends Model {
	...
	public function update_save_as($post_data)
	{
		...
		/*if(trim($currently_added) == "Vendor Address")
		{
			$this_id_record = $this->db->query("select * from " . DB_PREFIX . "address_lc WHERE id = '".$id."'");
			//print_r($this_id_record);
			$pos = strpos($this_id_record->row['use_as'], "Vendor");
			if($pos > 0)
			{
				//echo 'i am in vendor if';
			}
			else
			{
				//echo 'i am in vendor else';
				$total_address = $this_id_record->row['use_as'].' Vendor Address,';
	...
	public function update_current_address($post_data){
		...
		//echo "<pre>";print_r($query_address);echo "</pre>";
		//echo "<pre>";print_r($post_data);echo "</pre>";
