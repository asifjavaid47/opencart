...
class ModelCustomuserdetailCompanydetail extends Model {
	...
	public function add_compant_detail($cid , $post_data , $banner , $logo){
		...
		if($qry->num_rows > 0){
				//$this->db->query("UPDATE " . DB_PREFIX . "address_lc SET  address = '".$this->db->escape(isset($post_data['company_address'])?$this->db->escape($post_data['company_address']):'') ."'  , new_address = '".$this->db->escape(isset($post_data['address_1'])?$this->db->escape($post_data['address_1']):'') ."' , door_number = '".$this->db->escape(isset($post_data['company_doornumber'])?$this->db->escape($post_data['company_doornumber']):'') ."'   , floor = '".$this->db->escape(isset($post_data['company_floor'])?$this->db->escape($post_data['company_floor']):'') ."'  , apartment = '".$this->db->escape(isset($post_data['company_apartment'])?$this->db->escape($post_data['company_apartment']):'') ."'  , 	postcode = '".$this->db->escape(isset($post_data['postcode'])?$this->db->escape($post_data['postcode']):'') ."'  , 	city = '".$this->db->escape(isset($post_data['city'])?$this->db->escape($post_data['city']):'') ."'  , 	country = '".$this->db->escape(isset($post_data['country_id'])?$this->db->escape($post_data['country_id']):'') ."'  , 	province = '".$this->db->escape(isset($post_data['provience_id'])?$this->db->escape($post_data['provience_id']):'') ."' , use_as = ' Company Address'  WHERE customer_id = '" . (int)$cid . "' and get_from = 'company detail'");
		}else{
			...
			if(!empty($post_data['company_doornumber'])){
				...
				else
				{
					$this->db->query("UPDATE " . DB_PREFIX . "address_lc SET  use_as = ' Delivery Address, Contact Address,'  WHERE customer_id = '" . (int)$cid . "' and get_from = 'user detail'  ");
				               ...
				               //$this->db->query("INSERT INTO ".DB_PREFIX."address_lc SET address = '".$this->db->escape(isset($post_data['company_address'])?$this->db->escape($post_data['company_address']):'') ."'  , new_address = '".$this->db->escape(isset($post_data['address_1'])?$this->db->escape($post_data['address_1']):'') ."' , door_number = '".$this->db->escape(isset($post_data['company_doornumber'])?$this->db->escape($post_data['company_doornumber']):'') ."'   , floor = '".$this->db->escape(isset($post_data['company_floor'])?$this->db->escape($post_data['company_floor']):'') ."'  , apartment = '".$this->db->escape(isset($post_data['company_apartment'])?$this->db->escape($post_data['company_apartment']):'') ."'  , 	postcode = '".$this->db->escape(isset($post_data['postcode'])?$this->db->escape($post_data['postcode']):'') ."'  , 	city = '".$this->db->escape(isset($post_data['city'])?$this->db->escape($post_data['city']):'') ."'  , 	country = '".$this->db->escape(isset($post_data['country_id'])?$this->db->escape($post_data['country_id']):'') ."'  , 	province = '".$this->db->escape(isset($post_data['provience_id'])?$this->db->escape($post_data['provience_id']):'') ."' ,get_from = 'company detail' ,  customer_id = '" . (int)$cid . "' ");
