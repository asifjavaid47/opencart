...
class ModelCatalogCategory extends Model {
    ...
    public function addQuestion($data) {
       ...
       $this->db->query("insert into oc_question(customer_id,product_id,question,date_time,report_id) values('" . $data['customer_id'] . "', '" . $data['product_id'] . "', '" . $data['question'] . "', '" . $data['date_time'] . "', '" . $data['report_id'] . "' )");
    }
    ...
    public function add_answer($info) {
        ...
        //exit;
        $this->db->query("INSERT INTO " . DB_PREFIX . "answer (question_id,vendor_id,answer) VALUES (" . (int) $info['question_id'] . ", " . $this->db->escape($info['vendor_id']) . " , '" . $this->db->escape($info['answer']) . "' )");
    }
    ...
    public function get_level_one_category($id) {
        ...
        $query = $this->db->query("SELECT a.* , b.*
										...
										where a.parent_id = " . $id . " and b.language_id  = 1
  ...
  us.`category_id`,
  COUNT(cd.category_id) AS cnt,cd.name 
FROM
  `oc_userhistory_lc` us
  INNER JOIN  `oc_category_description` cd ON us.`category_id`= cd.category_id
WHERE ip_address = '$ip' 
