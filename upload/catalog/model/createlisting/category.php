...
class ModelCreatelistingCategory extends Model {
	...
	/*public function product_listing_type(){
			...
			$query = $this->db->query("SELECT * FROM product_commission_lc");
			return $query;
	}*/
	...
	public function add_vendor_product($vendor_id , $info){
		...
		if($info['flag_product_id']){
			 ...
			 //$this->db->query("UPDATE ".DB_PREFIX."product_description SET  name = '".$this->db->escape($info['prod_title'])."'  , description =  '".$this->db->escape(htmlspecialchars($info['prod_desc']))."' , tag = '".$this->db->escape(isset($info['prod_tags'])?$this->db->escape($info['prod_tags']):'')."' , short_desc = '".$this->db->escape(isset($info['prod_short_desc'])?$this->db->escape($info['prod_short_desc']):'')."'  where product_id = ".$info['flag_product_id']." ");
			...
			$this->db->query("delete from ".DB_PREFIX."product_image where product_id = ".(int)$info['flag_product_id']." ");
			 ...
			 //$require_shipping = $info['require_shipping']?$info['require_shipping']:'';
			 ...
			 //$use_porcantidad_or_own_shipping = $info['use_porcantidad_or_own_shipping']?$info['use_porcantidad_or_own_shipping']:'';
			// $buyer_pay_or_free_shipping = $info['buyer_pay_or_free_shipping']?$info['buyer_pay_or_free_shipping']:'';
			// $own_ship_price = $info['own_ship_price']?$info['own_ship_price']:'';
			// $shipping_radius = $info['shipping_radius']?$info['shipping_radius']:'';

			 
			// $this->db->query("INSERT INTO shipping_method_lc (require_shipping_method,zone_delivery , width , height , depth , weight , width_attr , height_attr , depth_attr , weight_attr ,	porcantidad_vendor_shipping , product_id , vendor_id) VALUES ('".$this->db->escape($require_shipping)."', '".$this->db->escape($zone_delivery)."'  , '".$this->db->escape($input_shipping_width)."', '".$this->db->escape($input_shipping_height)."' , '".$this->db->escape($input_shipping_depth)."'   , '".$this->db->escape($input_shipping_weight)."'      , '".$this->db->escape($option_shipping_width)."'    , '".$this->db->escape($option_shipping_height)."'   , '".$this->db->escape($option_shipping_depth)."'   , '".$this->db->escape($option_shipping_weight)."' , '".$this->db->escape($porcantidad_pay_shipping_checkbox)."' ,  ".(int)$info['flag_product_id']."   , ".(int)$vendor_id." )");
		...
		else
		{
			 ...
			 //echo "INSERT INTO ".DB_PREFIX."product (seller_id,model,sku,product_condition,price,image,stock_status_id,quantity, date_available, minimum, status, date_added, approve , listing_type) VALUES (".(int)$vendor_id." ,  '".$this->db->escape($info['prod_title'])."' , '".$this->db->escape($skunumber)."' , '".$this->db->escape($info['prod_condition'])."', ".$this->db->escape($price_format).", '".$this->db->escape($product_main_image)."', ".$stock_status.",".$this->db->escape($info['avail_stock']).", '".$this->db->escape(date('Y-m-d'))."' , ".$this->db->escape($info['minimum_buy'])." , ".$prod_status." , '".$this->db->escape(date('Y-m-d H:i:s'))."' ,".$prod_status." , '1')";
			 ...
			 //$this->db->query("INSERT INTO ".DB_PREFIX."product_description (product_id,language_id,name,description,tag , short_desc ) VALUES (".(int)$last_id.",".$this->db->escape($language_id).", '".$this->db->escape($info['prod_title'])."', '".$this->db->escape(htmlspecialchars($info['prod_desc']))."' , '".$this->db->escape(isset($info['prod_tags'])?$this->db->escape($info['prod_tags']):'')."'   , '".$this->db->escape(isset($info['prod_short_desc'])?$this->db->escape($info['prod_short_desc']):'')."' )");
             ...
             $this->db->query("INSERT INTO shipping_method_lc (vendor,buyer,xenvios,zone_delivery , width , height , depth , weight , width_attr , height_attr , depth_attr , weight_attr ,	porcantidad_vendor_shipping , product_id , vendor_id) VALUES ('".$this->db->escape($new_vendor_shipping)."', '".$this->db->escape($new_buyer_shipping)."', '".$this->db->escape($new_xenvios_shipping)."', '".$this->db->escape($zone_delivery)."'  , '".$this->db->escape($input_shipping_width)."', '".$this->db->escape($input_shipping_height)."' , '".$this->db->escape($input_shipping_depth)."'   , '".$this->db->escape($input_shipping_weight)."'      , '".$this->db->escape($option_shipping_width)."'    , '".$this->db->escape($option_shipping_height)."'   , '".$this->db->escape($option_shipping_depth)."'   , '".$this->db->escape($option_shipping_weight)."' , '".$this->db->escape($porcantidad_pay_shipping_checkbox)."' ,  ".(int)$last_id."   , ".(int)$vendor_id." )");
			 //$this->db->query("INSERT INTO shipping_method_lc (require_shipping_method,porcantidad_or_own_shipping,	buyer_pay_or_free_shipping,own_shipping_price,radius , product_id , vendor_id) VALUES ('".$this->db->escape($require_shipping)."', '".$this->db->escape($use_porcantidad_or_own_shipping)."', '".$this->db->escape($buyer_pay_or_free_shipping)."' , '".$this->db->escape($own_ship_price)."'   , '".$this->db->escape($shipping_radius)."' , ".(int)$last_id."   , ".(int)$vendor_id." )");
