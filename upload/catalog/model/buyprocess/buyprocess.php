...
class ModelBuyprocessBuyprocess extends Model {
        ...
        if ($quantity <= 1) {
            ...
            for ($i = 0; $i < $q->num_rows; $i++) {
                if ($quantity >= $q->rows[$i]['quantity']) {
                    /* $cummulative_quantity = $q->rows[$i]['quantity'];
                      $cummulative_price = $q->rows[$i]['price'];
                      $product_price = $cummulative_price/$cummulative_quantity; */
    ...
    public function addOrder($data) {
        ...
        /* custom table oc_order_lc
          $q1 = $this->db->query("insert into `oc_order_lc` values('', '".$inv_prefix."', '".$data['customer_id']."', '0', '".$data['firstname']."', '".$data['lastname']."', '".$data['email']."', '".$data['telephone']."', '', '".$data['firstname']."', '".$data['lastname']."', '', '".$data['payment_address1']."', '', '".$data['payment_city']."', '".$data['payment_postcode']."', '', '".$data['payment_country_id']."', '', '".$data['payment_zone_id']."', '', '', '', '', '".$data['firstname']."', '".$data['lastname']."', '', '".$data['shipping_address1']."', '', '".$data['shipping_city']."', '".$data['shipping_postcode']."', '', '".$data['shipping_country_id']."', '', '".$data['shipping_zone_id']."', '', '', '".$data['require_shipping_method']."', '".$data['porcantidad_or_own_shipping']."', '".$data['buyer_pay_or_free_shipping']."', '', '".$data['shipping_charges']."', '".$data['total']."', '1', '".$data['vendor_id']."', '2', 'USD', '1.00', '".$data['ip']."', '".$datetime."', '".$datetime."' ) "); */
    ...
    public function sale_purchase_record($data) {

        $qry = $this->db->query("select * from `oc_product` where `product_id` = " . $data['product_id_final'] . " ");
        $qry = $qry->row['quantity'];
        $net_quantity = $qry - $data['quantity'];
        $this->db->query("update `oc_product` set `quantity` = " . $net_quantity . " where `product_id` = " . $data['product_id_final'] . "  ");
        $sub_total = explode("&", $data['subtotal']);
        $sub_total = $sub_total[0];

        $this->db->query("INSERT INTO `oc_seller_lc` (quantity,subtotal,total,shiping_charges,shiping_address,payment_method,product_image,save_amount,product_id,buyer_id)VALUES('" . $data['quantity'] . "','" . $sub_total . "','" . $data['total'] . "','" . $data['shiping_charges'] . "','" . $data['ship_address'] . "','" . $data['paymentmethod'] . "','" . $data['product_image'] . "','" . $data['save_amount'] . "','" . $data['product_id_final'] . "','" . $data['buyer_id'] . "')");

        $this->db->query("INSERT INTO `oc_buyer_lc` (quantity,subtotal,total,shiping_charges,shiping_address,payment_method,product_image,save_amount,product_id,seller_id)VALUES('" . $data['quantity'] . "','" . $sub_total . "','" . $data['total'] . "','" . $data['shiping_charges'] . "','" . $data['ship_address'] . "','" . $data['paymentmethod'] . "','" . $data['product_image'] . "','" . $data['save_amount'] . "','" . $data['product_id_final'] . "','" . $data['seller_id'] . "')");
    }

    public function get_seller_data($product_id) {

        $query = $this->db->query(" SELECT seller_id,vd.`id`,oc.`customer_id` FROM oc_product op INNER JOIN oc_vendordetail_lc vd  ON op.`seller_id` = vd.`id` INNER JOIN oc_customer oc ON vd.`customer_id`=oc.`customer_id`WHERE op.product_id = '" . $product_id . "' ");
