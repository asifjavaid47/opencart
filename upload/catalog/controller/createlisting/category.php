...
class ControllerCreatelistingCategory extends Controller {
    ...
    public function index() {
	   ...
	   $check_vendor_id = $this->model_createlisting_category->get_vendor_id();
		...
		//$data['product_listing_type'] = $this->model_createlisting_category->product_listing_type();
		...
		if(!empty($get_customer_id_sees)){
			foreach($get_customer_id_sees as $custaddr){
			   ...
			   // echo "<pre>";print_r($get_city);echo "</pre>";
				//echo "<pre>";print_r($get_province);echo "</pre>";
	...
	public function check_category(){
		...
		//echo "<pre>";print_r($data['get_sub_category']); echo "</pre>";
		//exit;
	}
	...
	public function check_second_sub_category(){
		...
		//echo "<pre>";print_r($get_sub_category); echo "</pre>";
		//exit;
	}
	...
	public function upload_product_main_img(){
		    ...
		    //$rrrr = $this->request->files['file']['name'];
			...
			if (isset($_FILES["file"] ) && is_uploaded_file($this->request->files['file']['tmp_name'])) {
				...
				//echo json_encode(array('main_image'=>$data['product_main_image'],'encryptedname'=>$moveto));
	...
	function resize($width, $height , $image_name){
			  ...
			  $w = ceil($width / $ratio);
			  /* new file name */
			  //$path = 'image/'.$width.'x'.$height.'_'.$_FILES['file']['name'];
			 // $path = 'image/'.$width.'x'.$height.'_'.$image_name;
			  			 
			  $get_extension = pathinfo($image_name, PATHINFO_EXTENSION);
			  $explode_from_extention = explode(".".$get_extension,$image_name);
			  //print_r($explode_from_extention[0].'-'.$width.'x'.$height.'.'.$get_extension);
