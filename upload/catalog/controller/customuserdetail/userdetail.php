...
class ControllerCustomuserdetailUserdetail extends Controller {
    ...
    public function index() {
        ...
        //echo $cid = $this->customer->getId();exit;
	...
	public function user_detail(){
				...
				//$this->response->redirect($this->url->link('customuserdetail/vendordetail'));
	}
		...
		$data['get_country'] = $get_country;
              ...
              if(!empty($get_customer_vendor_address->row['floor'])){
               ...
               if(isset($get_customer_vendor_address->row['new_address'])){
                    ...
                    $complete_address_v =  $vendor_new_address->row['v_address'];
               }*/
    ...
    public function check_taxid(){
        ...
        if ($status == 0) {
            //echo $this->language->get('error_userdet_taxid');
			echo 'not valid';
        } else if ($status == 1) {
			...
			//echo "<pre>"; print_r($record_for_tax_id); echo "</pre>";
			
			if ($record_for_tax_id->rows[0]['number_of_tax_ids'] > 0) {
				//echo $this->language->get('error_userdet_taxid_already_exists');
				echo 'already exists';
			}
        }
	}
...
//			$this->error['zone'] = $this->language->get('error_userdet_zone');
//		}
