...
class ControllerProductdetailsproductdtls extends Controller {
    ...
    public function sendemail($emailToSeller, $prodict_id, $question_or_answer, $flag) {
        ...
        if ($flag == 'question') {
            ...
            $message .= 'http://porcantidad.leadconcept.biz/upload/index.php?route=Productdetails/productdtls&zamr=$0&xqtn=1&pxs=' . $prodict_id . "\n\n";
        } else {
    ...
    public function reported_email_to_reporter($prodict_id, $email_of_reporter) {
        //echo $this->config->get('config_mail_protocol');
        $message = sprintf('Your have reported this product For further details click the link below') . "\n\n";
        $message .= 'http://porcantidad.leadconcept.biz/upload/index.php?route=dashboard/dashboardreport&xds='.$prodict_id .''."\n\n";
        $email_to = $email_of_reporter; //"bilalahmadue@gmail.com";//
        $mail = new Mail();
        $mail->protocol = $this->config->get('config_mail_protocol');
        $mail->parameter = $this->config->get('config_mail_parameter');
        $mail->smtp_hostname = $this->config->get('config_mail_smtp_hostname');
       ...
       public function reported_email_to_seller($emailToSeller, $prodict_id, $email_of_reporter, $reportername) {
        //echo $this->config->get('config_mail_protocol');
    ...
    public function save_reporting(){
        if ($this->customer->isLogged()) {
            ...
            //$data['seller_email'] = "asifjavaid74@gmail.com";
            $reporter_email = $this->customer->getEmail();
            //$reporter_email = "asifjavaid74@gmail.com";
            $reporter_name = $this->customer->getFirstName() . " " . $this->customer->getLastName();
            $this->model_catalog_category->save_reporting($data);
            $this->reported_email_to_seller($data['seller_email'],$data['product_id'], $reporter_email, $reporter_name);
            //$this->reported_email_to_reporter($data['product_id'],$reporter_email);
            echo 1;
        } else {
            echo 0;
        }
